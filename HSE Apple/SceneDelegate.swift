...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        let isAuthorized = userDefault.object(forKey: "isAuthorized") as? Bool ?? false
        if isAuthorized {
            startApp()
        } else {
            startLogin()
    ...
    func startApp() {
        let startVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(identifier: "ApplicationViewController")
